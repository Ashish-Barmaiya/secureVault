...
import { Vault, LogOut, Shield, Key, FileText, ShieldCheck, ShieldAlert } from "lucide-react";
...
export default function HeirDashboard() {
  ...
  // We can still fetch fresh data on mount to ensure sync, or rely on Redux.
  // Best practice: Fetch on mount and update Redux.
  useEffect(() => {
    if (!isLoggedIn) {
    ...
    <div className="min-h-screen bg-[#0f172a] text-white">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
        <div className="mb-12">
          ...
          <p className="text-slate-400">Manage your inherited assets and verify your identity.</p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-12">
          ...
          <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6 backdrop-blur-sm">
            <div className="flex items-center gap-4 mb-4">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center ${heirData?.twoFactorEnabled ? "bg-green-500/10 text-green-400" : "bg-orange-500/10 text-orange-400"}`}>
                {heirData?.twoFactorEnabled ? <ShieldCheck className="w-5 h-5" /> : <ShieldAlert className="w-5 h-5" />}
              </div>
              <div>
                <h3 className="font-semibold text-white">2FA Security</h3>
                <p className={`text-sm ${heirData?.twoFactorEnabled ? "text-green-400" : "text-orange-400"}`}>
                  {heirData?.twoFactorEnabled ? "Enabled & Secure" : "Action Required"}
                </p>
              </div>
            </div>
        ...
        <div className="bg-slate-900/30 border border-slate-800 border-dashed rounded-2xl p-12 text-center">
          ...
          <h3 className="text-lg font-medium text-white mb-2">No Vaults Assigned Yet</h3>
          <p className="text-slate-400 max-w-md mx-auto">
            When a user designates you as an heir and grants access to their vault, it will appear here. 
            Ensure your email matches the one provided by the vault owner.
          </p>
        </div>
      </main>
